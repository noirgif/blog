...
主要原理是在网页上用 javascript 访问 Express.js 提供的 API 获取数据，以生成表格以及图表。图表部分由于用了特定的复杂查询（如求和啊，过滤之类的），因此整个程序变得不简洁，也就没放上去。

总之这个架构是这样：HTTP 服务器提供 raw data ，再由 Javascript 进行渲染。


虽然 Disquz 评论显示不出来。老奎！这样是不行的！

如果像了解老奎是怎样的一个人，还是推荐去[他的推](https://twitter.com/quininers) 逛逛。
